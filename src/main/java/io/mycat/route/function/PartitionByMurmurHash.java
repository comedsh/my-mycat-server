...
//			}
	...
	/**
	 * adds the printing information for understanding.
	 * 
	 * 用 eclipse 打印不出想要的结果，需要将 eclipse 的 System.out 重定向到文件中才可以观察得到。
	 * 
	 */
	private void generateBucketMap(){
		
		hash=Hashing.murmur3_32(seed);//计算一致性哈希的对象 -> 对应的就是 IntHashCode
		
		for(int i=0;i<count;i++){//构造一致性哈希环，用TreeMap表示
			
			StringBuilder hashName=new StringBuilder("SHARD-").append(i);
			
			String log = "for server node "+ i +": ";
			
			for(int n=0,shard=virtualBucketTimes*getWeight(i);n<shard;n++){
				
				String virtualnode = hashName.append("-NODE-").append(n).toString();
				
				HashCode code = hash.hashUnencodedChars( virtualnode );
				
				log += virtualnode + "(" + code.hashCode() + "), ";
				
				bucketMap.put( code.asInt(), i );
...
//	}
	...
	public Integer calculate(String columnValue) {
		...
		/** my comments: 当该 columnValue 计算出来的 point 恰好位于一致性 hash 环上的 最后一个虚拟节点 到 起点 的时候，tail 返回为空，这个时候，需要将该数据存放在起点位置。**/ 
		if (tail.isEmpty()) {
	...
	/**
	 * 添加了 calculatedPoints 用于统计由不同的 ID 计算所得到的重复的 point。(注意，添加了 calculatedPoints 以后程序执行会非常的慢，可以注释掉)
	 * 
	 * 测试过程中找到了 duplicated points => point ¹<---->ⁿ ID 核心对应关系；进一步验证参考 {@link MyPartitionByMurmurHashTest#testCalculatedDuplicatePoints()}
	 * 
	 * @throws IOException
	 */
	private static void hashTest() throws IOException{
		...
		List<Integer> calculatedPoints = new ArrayList<Integer>(); // 缓存已经计算得出的 hash 环中的 point
		
		for(int i=100_0000;i<total+100_0000;i++){//假设分片键从100万开始
			c++;
			
			HashFunction hashFunc = Hashing.murmur3_32( 0 ); // assume the seed is 0;
			
			Integer point = hashFunc.hashUnencodedChars( Integer.toString(i) ).asInt();
			
			if( calculatedPoints.contains(point) ){
				
				int index = calculatedPoints.indexOf( point ); // 第几个 i 的位置？
				
				int duplicatedWith = 100_0000 + index; // the i
				
				System.out.println("found duplicated calcualted point: " + point +" when calculating " + i + ", it's duplicated with "+ duplicatedWith );
				
			}
			
			calculatedPoints.add( point ); // 如果有重复，保存重复的 point，这样 list 中的 point 的位置可以对应 i 的位置。			
