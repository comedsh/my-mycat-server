...
public class PartitionByCRC32PreSlot extends AbstractPartitionAlgorithm
    ...
    private Properties genarateP() {
        ...
        /** 详细记录每一个 data node 所包含的 slots 的编号信息 **/
        Properties prop = new Properties();
    ...
    @Override public Integer calculate(String columnValue) {
        ...
        int slotSize = DEFAULT_SLOTS_NUM / count; /** 每一个 data node 包含多少 slots **/
        
        /**
         * 得到第几个物理节点；
         * 比如 count = 2, slot = 91232, slot size = ( DEFAULT_SLOTS_NUM ) 102400 / 2 = 51200; => index = 91232 / 51200 = 1 -> 既表示用第二台 data node 进行存储，从 0 开始编号。
         * 比如 count = 2, slot = 91232, slot size = ( DEFAULT_SLOTS_NUM ) 102400 / 3 = 34133; => index = 91232 / 34133 = 2 -> 既表示用第三台 data node 进行存储，从 0 开始编号。
         * 
         **/
        int index = slot / slotSize;
        
        /**
         * 当 count = 1 的时候，会出现如下 if 的情况，并返回 0
         */
        if (slotSize * count != DEFAULT_SLOTS_NUM && index > count - 1) {
            index = (count - 1);
        }
        
        return index; // 返回需要存储的物理节点 data node。
    }
